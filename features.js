document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!Cart) {
    console.error(
      "Achija: cart.js must be loaded before features.js and must expose window.Cart",
    );
    return;
  }
  ...
  uiRoot.innerHTML = `
        ...
        <div id="search-overlay" class="fixed inset-0 bg-[#001233]/97 backdrop-blur-xl z-[70] flex flex-col items-center justify-center opacity-0 pointer-events-none transition-opacity duration-500">
            ...
            <div class="w-full max-w-3xl px-6 md:px-8">
                <p class="font-['Noto_Serif'] text-[#c4a96e] text-lg md:text-xl mb-4 text-center">What are you looking for?</p>
                <div class="relative">
                    <input id="search-input" type="text" placeholder="Search the collection..." autocomplete="off"
                        class="w-full bg-transparent border-b border-[#44464E] focus:border-[#c4a96e] outline-none text-2xl md:text-3xl font-light text-white py-4 font-['Inter'] placeholder:text-white/20 transition-colors">
        ...
        <div id="cart-drawer" class="fixed top-0 right-0 h-full w-full sm:w-[420px] bg-[#00091f]/97 backdrop-blur-md z-[65] transform translate-x-full transition-transform duration-500 shadow-2xl border-l border-[#44464E]/20 flex flex-col">
            <div class="flex justify-between items-center p-6 md:p-8 border-b border-[#44464E]/20">
                <div class="flex items-center gap-3">
                    ...
                    <span id="cart-header-count" class="bg-[#c4a96e]/80 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center"></span>
                </div>
            ...
            <div class="p-6 md:p-8 border-t border-[#44464E]/20">
                <div class="flex justify-between items-center mb-5">
                    <span class="font-['Inter'] text-[#c4a96e]/70 tracking-widest text-xs uppercase">Subtotal</span>
                    <span id="cart-total" class="font-['Noto_Serif'] text-xl text-white">₹0</span>
                </div>
                <div id="cart-empty-msg" class="hidden text-center text-[#c4a96e]/40 font-['Inter'] text-sm pb-4">Your collection is empty.</div>
                <button id="checkout-btn" class="w-full py-4 bg-transparent border border-[#c4a96e] hover:shadow-[0_0_25px_rgba(196,169,110,0.25)] transition-all flex items-center justify-center gap-2 group cursor-pointer">
                    <span class="font-['Inter'] uppercase tracking-[0.2em] text-[0.75rem] text-[#c4a96e] font-medium group-hover:text-white group-hover:drop-shadow-[0_0_8px_rgba(196,169,110,0.8)] transition-all">Proceed to Checkout</span>
                    <span class="material-symbols-outlined text-[#c4a96e]/60 group-hover:text-white text-lg transition-colors">arrow_forward</span>
                </button>
                <p class="text-center text-[#c4a96e]/30 text-[0.6rem] tracking-widest uppercase mt-3 font-['Inter']">Secure Payment via Razorpay</p>
            </div>
        </div>
        ...
        <div id="qv-modal" class="fixed inset-x-4 md:inset-x-auto md:left-1/2 md:-translate-x-1/2 top-1/2 -translate-y-1/2 w-auto md:w-[820px] bg-[#00091f] border border-[#44464E]/30 z-[85] opacity-0 pointer-events-none transition-all duration-400 scale-95 p-0 shadow-[0_40px_100px_rgba(0,0,0,0.6)] max-h-[90vh] overflow-y-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2">
                ...
                <div class="p-6 md:p-10 flex flex-col justify-center space-y-4">
                    <span class="text-[#c4a96e]/50 text-xs tracking-[0.2em] uppercase font-['Inter']" id="qv-cat"></span>
                    <h2 id="qv-name" class="font-['Noto_Serif'] text-2xl md:text-3xl text-white leading-tight"></h2>
                    <p id="qv-price" class="text-xl premium-gradient-text font-['Noto_Serif']"></p>
                    <p id="qv-desc" class="text-[#c4a96e]/60 text-sm leading-relaxed font-['Inter']"></p>
                    <div class="grid grid-cols-2 gap-3 text-xs font-['Inter'] text-[#c4a96e]/50 pt-2 border-t border-[#44464E]/20">
                    ...
                    <div class="flex flex-col gap-3 pt-2">
                        <button id="qv-add-cart" data-id="" class="w-full py-4 border border-[#c4a96e]/60 hover:border-white text-[#c4a96e] hover:text-white font-['Inter'] uppercase tracking-[0.15em] text-xs transition-all hover:shadow-[0_0_20px_rgba(196,169,110,0.2)] flex items-center justify-center gap-2">
                            <span class="material-symbols-outlined text-base">shopping_bag</span>
                            Add to Collection
                        </button>
                        <a id="qv-view-btn" href="#" class="w-full py-3 text-center font-['Inter'] uppercase tracking-[0.15em] text-[0.7rem] text-[#c4a96e]/40 hover:text-white transition-colors">
                            View Full Details →
                        </a>
                    </div>
                </div>
        ...
        <div id="toast" class="fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] opacity-0 pointer-events-none transition-all duration-400 translate-y-4">
            <div class="bg-[#001233] border border-[#c4a96e]/20 text-white font-['Inter'] text-sm tracking-wide px-6 py-3 flex items-center gap-3 shadow-2xl">
                <span class="material-symbols-outlined text-[#c4a96e] text-base">check_circle</span>
        ...
        <!-- Cart Badge on nav -->
        <style>
            .cart-badge { display:none; position:absolute; top:-6px; right:-6px; background:#c4a96e; color:#000; font-size:10px; font-weight:700; width:16px; height:16px; border-radius:50%; align-items:center; justify-content:center; }
  ...
  function updateCartBadges() {
    ...
    if (hc) {
      hc.textContent = count;
      hc.style.display = count > 0 ? "flex" : "none";
    }
  }
  ...
  if (searchInput) {
    searchInput.addEventListener("input", () => {
      ...
      const matches = allProducts
        ...
        .slice(0, 8);
      if (matches.length === 0) {
        results.innerHTML = `<p class="text-[#c4a96e]/30 text-sm font-['Inter'] text-center pt-4">No results found.</p>`;
        return;
      }
      matches.forEach((p) => {
        ...
        a.className =
          "flex items-center gap-4 p-3 hover:bg-white/5 transition-colors group";
        a.innerHTML = `
                    <img src="${p.image}" class="w-12 h-12 object-cover grayscale opacity-60 flex-shrink-0">
                    <div>
                        <p class="font-['Noto_Serif'] text-white text-sm group-hover:text-[#c4a96e] transition-colors">${p.name}</p>
                        <p class="font-['Inter'] text-[#c4a96e]/40 text-xs tracking-widest uppercase">${p.category} · ${fmt(p.price)}</p>
                    </div>
                `;
  ...
  const isProductDetails =
    path.includes("product_details") || path.includes("product-details");
  ...
  if (isCollections) {
    ...
    function renderActiveFilters() {
      ...
      if (activeCategory !== "All")
        tags.push({ label: `Category: ${activeCategory}`, type: "cat" });
      if (maxPrice < 200000)
        tags.push({ label: `Under ${fmt(maxPrice)}`, type: "price" });
      ...
      tags.forEach((tag) => {
        ...
        span.className =
          'filter-tag-anim inline-flex items-center gap-1.5 bg-[#c4a96e]/20 border border-[#c4a96e]/30 text-[#c4a96e] font-["Inter"] text-[0.65rem] tracking-widest uppercase px-3 py-1.5 cursor-pointer hover:bg-red-900/30 hover:border-red-500/30 hover:text-red-200 transition-all duration-300';
      ...
      priceRange.addEventListener("input", () => {
        ...
        if (priceDisplay) {
          priceDisplay.textContent =
            maxPrice >= 200000 ? "₹2,00,000+" : fmt(maxPrice);
        }
      });
    }
    ...
    function renderProductsPage(page) {
      ...
      setTimeout(() => {
        ...
        if (pageItems.length === 0) {
          productGrid.innerHTML = `
                        <div class="col-span-3 py-24 text-center text-[#c4a96e]/40 font-['Inter'] tracking-widest uppercase text-sm">
        ...
        pageItems.forEach((prod, index) => {
          ...
          article.innerHTML = `
                        <div class="relative aspect-[4/5] overflow-hidden mb-5 shadow-[0_20px_50px_rgba(0,0,0,0.3)] bg-[#060d1f]">
                            <div class="absolute inset-0 bg-[#c4a96e]/10 opacity-0 group-hover:opacity-100 transition-opacity duration-700 z-0"></div>
                            ...
                            <div class="absolute inset-0 z-20 opacity-0 group-hover:opacity-100 border border-[#c4a96e]/20 transition-opacity duration-500 pointer-events-none"></div>
                            <div class="absolute bottom-0 left-0 right-0 h-1/3 z-20 bg-gradient-to-t from-[#c4a96e]/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none"></div>
                            <!-- Overlay buttons -->
                            <div class="absolute inset-0 z-30 flex flex-col items-center justify-end pb-5 gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                ...
                                <button class="atc-btn bg-[#c4a96e]/90 backdrop-blur-sm border border-[#c4a96e]/40 text-white font-['Inter'] text-[0.6rem] tracking-[0.15em] uppercase px-5 py-2.5 hover:bg-[#c4a96e]/80 transition-all" data-prod-id="${prod.id}">
                        ...
                        <div class="space-y-1">
                            <h2 class="font-headline text-base md:text-lg text-white group-hover:premium-gradient-text transition-all duration-300">${prod.name}</h2>
                            <p class="font-label text-[#ab9462] text-[0.65rem] tracking-[0.15em] text-on-surface-variant uppercase">${prod.category} • ${fmt(prod.price)}</p>
                        </div>
                    `;

          // Navigate to product detail (not when clicking action buttons)
          article.addEventListener("click", (e) => {
            if (e.target.closest(".qv-btn") || e.target.closest(".atc-btn"))
              return;
    ...
    function renderPagination() {
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        if (i === currentPage) {
          span.className =
            "text-white border-b border-white pb-1 cursor-default";
        } else {
          span.className =
            "text-outline hover:text-white transition-colors cursor-pointer";
          span.onclick = () => {
            currentPage = i;
      ...
      shuffled.forEach((prod, index) => {
        ...
        div.innerHTML = `
                    <div class="aspect-[3/4] overflow-hidden relative mb-5 bg-[#060d1f]">
                        ...
                        <div class="absolute bottom-0 left-0 right-0 h-1/3 z-20 bg-gradient-to-t from-[#c4a96e]/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none"></div>
                    </div>
                    <div class="space-y-1">
                        <h3 class="text-lg font-headline text-white/90 group-hover:text-[#c4a96e] transition-colors duration-300">${prod.name}</h3>
                        <p class="text-[#c4a96e]/60 font-body text-xs tracking-widest uppercase">${fmt(prod.price)}</p>
                    </div>
                `;
    ...
    document.querySelectorAll("img.grayscale").forEach((img, i) => {
      ...
      if (container && allProducts[i]) {
        ...
        container.addEventListener(
          "click",
          () =>
            (window.location.href = `product_details.html?id=${allProducts[i].id}`),
        );
      }
    });
  }
